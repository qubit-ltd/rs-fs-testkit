...
fn test_synchronous_fixture_defaults_are_unsupported() {
    ...
    assert!(matches!(
        fixture.empty_directory_path(),
        Ok(FixtureSupport::Unsupported)
    ));
    assert!(matches!(
        fixture.symlink_path(),
        Ok(FixtureSupport::Unsupported)
    ));
    assert!(matches!(
...
fn test_asynchronous_fixture_defaults_are_unsupported() {
    ...
    assert!(matches!(
        poll_fixture_future(fixture.entry_identity(&path)),
        Ok(FixtureSupport::Unsupported)
    ));
    assert!(matches!(
...
/// Opaque fixture identities compare their provider-owned bytes exactly.
#[test]
fn test_fixture_entry_identity_compares_provider_bytes() {
    assert_eq!(
        FixtureEntryIdentity::new(vec![1, 2, 3]),
        FixtureEntryIdentity::new(vec![1, 2, 3])
    );
    assert_ne!(
        FixtureEntryIdentity::new(vec![1, 2, 3]),
        FixtureEntryIdentity::new(vec![3, 2, 1])
    );
}
