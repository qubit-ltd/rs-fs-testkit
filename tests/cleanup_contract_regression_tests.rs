...
/// A cleanup failure does not replace the panic raised by the contract body.
#[test]
fn test_body_panic_is_preserved_when_teardown_also_fails() {
    ...
    let message = payload
        ...
        .unwrap_or_default();
    assert!(
        message.contains("write contract"),
...
/// Cleanup keeps failed resources in the ledger so a later finish retries it.
#[test]
