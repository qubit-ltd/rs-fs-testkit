...
use qubit_fs::{
    AchievedAtomicity, AtomicityRequirement, CopyMethod, CopyOutcome, CopyStats,
    ...
    RenameOutcome, ServerSidePreference, WriteDisposition, WriteOutcome,
};
        ...
        if state.fault != MemoryFault::WriteDropsBytes {
                ...
                && state.fault != MemoryFault::AppendOverwrites
            {
                match state.entries.get(self.path.as_str()) {
                    ...
                    Some(Entry::Directory) | None => self.bytes.clone(),
                }
            } else {
                self.bytes.clone()
            };
