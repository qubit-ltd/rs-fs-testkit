...
use qubit_fs::{
    AchievedAtomicity, AsyncCopyOperationState, AtomicityRequirement, ChecksumPolicy,
    CopyConflictPolicy, CopyFailureState, CopyMethod, CopyOptions, CreateDirectoryOptions,
    DeleteOptions, DurabilityRequirement, FileKind, FileSystemCapability, FsErrorKind, FsOperation,
    ListOptions, PersistFailureState, PersistOptions, ReadOptions, RenameFailureState,
    RenameOptions, ResourceVersion, ServerSidePreference, TempDirectoryOptions, TempFileOptions,
...
use crate::{
    AsyncCopyCancellationStage, AsyncFileSystemFixture, FileSystemContract, FixtureSupport,
};
...
impl<'a> AsyncFileSystemContractSuite<'a> {
    ...
    pub async fn assert_copy(&mut self) {
        ...
        if !self.capable(FileSystemCapability::Copy) {
            ...
            let error = match self.fixture.file_system().begin_copy(
                ...
                Err(error) => error,
                Ok(_) => panic!("async copy contract: unadvertised copy preflight succeeded"),
            };
            assert_error_with_target(
                error.error(),
                FsErrorKind::UnsupportedCapability,
    ...
    async fn prepare_temp_options_parent(&mut self, relative: &str) -> Option<qubit_fs::Path> {
        if !self.capable(FileSystemCapability::CreateDirectory) {
            return None;
        }
