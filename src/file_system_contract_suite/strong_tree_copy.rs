...
// =============================================================================
//! Independently prepared tree copy publication guarantees.
...
impl FileSystemContractSuite<'_> {
    ...
    /// bytes.
    pub(super) fn check_strong_tree_copy(&mut self, id: ContractCheckId) -> Result<(), ContractFailure> {
        ...
        let (capability, options) = match scenario {
            ...
            _ => {
                return Err(ContractFailure::message_only("selected check is not a tree-copy guarantee").at(id));
            }
        };
        ...
        if !self.capable(capability) {
            ...
            let failure = match self.fixture.file_system().copy(&source, &target, options) {
                Err(failure) => failure,
                Ok(_) => {
                    return Err(ContractFailure::message_only("unavailable copy guarantee succeeded").at(id));
                }
            };
        ...
        for path in [&source_sub, &source_child, &target_sub, &target_child] {
            ...
            let FixtureSupport::Supported(exists) = observed else {
                self.context.record_check(
                    ...
                    ContractCheckOutcome::Unverified {
                        reason: "independent tree directory observation unavailable".to_owned(),
                    },
                );
                return Ok(());
            };
            verify_condition(exists, id, "tree source directory was not prepared")?;
        }
        ...
        let target_before = self.fixture.exists_out_of_band(&target).map_err(|error| {
        ...
        verify_condition(
            outcome.stats().bytes == bytes.len() as u64
                ...
                && outcome.stats().directories + outcome.stats().prefixes == 2
                && outcome.stats().skipped == 0
                && outcome.stats().failed == 0
                && outcome.stats().overwritten == 0,
            id,
        ...
        for path in [&source, &source_sub, &target, &target_sub] {
            ...
            verify_condition(
                ...
                id,
                "tree copy lost a source or target directory",
            )?;
        }
        for path in [&source_child, &target_child] {
            let observed = self
                .fixture
                .read_file(path)
                .map_err(|error| ContractFailure::with_source("strong copy final observation failed", error).at(id))?;
            verify_condition(
