...
impl FileSystemContractSuite<'_> {
    ...
    pub(super) fn check_representations(&mut self) -> Result<(), ContractFailure> {
        for id in [
            ContractCheckId::RepresentationEmpty,
            ContractCheckId::RepresentationSymlink,
        ] {
            self.check_representation_item(id)?;
        }
        Ok(())
    }
    ...
    pub(super) fn check_representation_item(&mut self, id: ContractCheckId) -> Result<(), ContractFailure> {
        let (capability, name) = match id {
            ...
            _ => {
                return Err(ContractFailure::message_only("selected entry is not a representation check").at(id));
            }
        };
        ...
        let prepared = if id == ContractCheckId::RepresentationEmpty {
            self.fixture.seed_empty_directory(&relative)
        } else {
