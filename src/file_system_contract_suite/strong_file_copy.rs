...
impl FileSystemContractSuite<'_> {
    ...
    /// bytes.
    pub(super) fn check_strong_file_copy(&mut self, id: ContractCheckId) -> Result<(), ContractFailure> {
        ...
        let (capability, options) = match scenario {
            ...
            _ => {
                return Err(ContractFailure::message_only("selected check is not a file-copy guarantee").at(id));
            }
        };
